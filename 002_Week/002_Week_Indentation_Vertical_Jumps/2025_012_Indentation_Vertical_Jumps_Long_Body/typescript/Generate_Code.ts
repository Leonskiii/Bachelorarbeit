...
const code_templates: Array<(word: string) => string> = [
    (w) => `const result = processData(config, ${w});`,
    (w) => `let buffer = allocate(maxSize, ${w});`,
    (w) => `db.update(table, { key: ${w} });`,
    (w) => `return transform(input, context, ${w});`,
    (w) => `cache.set(sessionId, ${w});`,
    (w) => `logger.warn(message, stack, ${w});`,
    (w) => `queue.push(priority, payload, ${w});`,
    (w) => `render(component, props, ${w});`,
    (w) => `stream.write(buffer, offset, ${w});`,
    (w) => `resolver.bind(context, handler, ${w});`,
];

function random_template(word: string): string {
...
export class Nested_Ifs extends Term {
    ...
    body_target_word_at(target_number: number): string {
        if (target_number == 1) return this.body_target_word;
        return (this.then_branch as Nested_Ifs).body_target_word_at(target_number - 1);
    }
}
