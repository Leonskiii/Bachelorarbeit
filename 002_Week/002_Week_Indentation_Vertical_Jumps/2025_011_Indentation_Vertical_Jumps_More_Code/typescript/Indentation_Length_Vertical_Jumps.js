...
//Indentation_Length_Vertical_Jumps_More_Code
let SEED = "42";
SET_SEED(SEED);
...
let experiment_configuration_function = (writer) => {
    return {
        ...
        task_configuration: (t) => {
            ...
            let target_body_word = if_statement.body_word_at(target_expression_number);
            ...
            html_string = html_string.replace(target_body_word, "<span style=\"background-color:red\">" + target_body_word + "</span>");
            t.expected_answer = correct_answer;
            t.do_print_task = () => {
                writer.clear_stage();
                const randomLeft = 5 + Math.floor(Math.random() * 50);
                const randomTop = 5 + Math.floor(Math.random() * 40);
                writer.print_string_on_stage(`<div class='sourcecode' style='position: absolute; left: ${randomLeft}%; top: ${randomTop}%;'>`
                    + html_string + "</div>");
            };
            t.accepts_answer = (s) => {
                return true;
