...
let SEED = "42";
SET_SEED(SEED);
function set_if_conditions(if_statement) {
    let starting_letter = [];
    let nouns = new Nouns();
    while (if_statement instanceof Nested_Ifs) {
        let word = nouns.get_random_word();
        while (starting_letter.includes(word[0]))
...
let experiment_configuration_function = (writer) => {
    return {
        ...
        task_configuration: (t) => {
            ...
            t.expected_answer = t.expected_answer = level.toString();
            t.has_pre_task_description = true;
            t.do_print_pre_task = () => {
                writer.clear_stage();
                writer.print_html_on_stage(`<h1>${target_word}</h1>`);
            };
            t.do_print_task = () => {
                writer.clear_stage();
