...
let experiment_configuration_function = (writer) => {
    return {
        ...
        task_configuration: (t) => {
            ...
            const indentation_type = t.treatment_value("Indentation_Type");
            const highlight_position = t.treatment_value("Highlight_Position");
            ...
            const marked_html = html_string.replace(new RegExp(`\\b${target_word}\\b`), `<span style="background-color:red">${target_word}</span>`);
            t.expected_answer = correct_answer;
            t.do_print_task = () => {
                writer.clear_stage();
                const randomLeft = 5 + Math.floor(Math.random() * 10);
                const randomTop = 10 + Math.floor(Math.random() * 40);
                writer.print_string_on_stage(`<div class='sourcecode' style='position:absolute;left:${randomLeft}%;top:${randomTop}%;'>${marked_html}</div>`);
            };
