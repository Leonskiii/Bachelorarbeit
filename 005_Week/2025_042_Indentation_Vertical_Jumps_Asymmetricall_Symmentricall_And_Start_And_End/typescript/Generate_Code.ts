...
export class Nested_Ifs extends Term {
    ...
    print_into(array: string[], indentation_depth: number, indentation_length: number) {
        array.push(
            " ".repeat(indentation_length * indentation_depth) +
    ...
    target_word(target_number: number, position: "First" | "Last"): string {
...
export function generate_If_Statement(
    indentation_depth,
    nesting_depth,
    indentation_length,
    ...
    function make_condition_of_char_length(target_chars: number): string {
        let terms = [];
        let total = 0;
    ...
    function make_symmetric_condition(): string {
    ...
    function make_asymmetric_condition(depth: number): string {
    ...
    function make_condition(depth: number): string {
        if (layout == "Symmetric") return make_symmetric_condition();
