...
export function generate_grid(num_rows, marked_row, length, density, // ← neu: Anzahl Kauderwelsch-Zeichen um die Zahl
seed) {
    ...
    for (let r = 0; r < num_rows; r++) {
        ...
        for (let i = 0; i < density; i++)
            left_noise += random_char(rng);
        for (let i = 0; i < density; i++)
            right_noise += random_char(rng);
        lines.push({
...
export function grid_to_html(lines) {
    ...
    return lines.map(line => {
        ...
        for (let i = 0; i < line.left_noise.length; i++)
            chars[noise_start + i] = line.left_noise[i];
        chars[line.num_col] = line.number.toString();
        for (let i = 0; i < line.right_noise.length; i++)
            chars[line.num_col + 1 + i] = line.right_noise[i];
