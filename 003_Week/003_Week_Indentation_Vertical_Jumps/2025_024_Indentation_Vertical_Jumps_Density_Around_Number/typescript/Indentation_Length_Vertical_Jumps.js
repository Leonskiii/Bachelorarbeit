...
let experiment_configuration_function = (writer) => {
    return {
        experiment_name: "Indentation_Vertical_Jumps_Random_At_End",
        seed: SEED,
        ...
        layout: [
            { variable: "Length", treatments: ["50", "100", "150", "200"] },
        ...
        task_configuration: (t) => {
            ...
            const { lines, marked_number } = generate_grid(num_rows, row, length, density, Math.floor(Math.random() * 100000));
            const center = 5;
            const distance = Math.abs((row + 1) - center);
            t.set_computed_variable_value("Distance_from_Center", distance.toString());
