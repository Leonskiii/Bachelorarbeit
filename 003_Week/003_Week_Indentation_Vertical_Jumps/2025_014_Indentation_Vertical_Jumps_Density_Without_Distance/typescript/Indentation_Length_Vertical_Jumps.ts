...
export function set_if_conditions_on_nouns_each_starting_with_different_letter(if_statement: Term) {
    ...
    while (if_statement instanceof Nested_Ifs) {
        ...
        while (starting_letter.includes(next_words[0])) next_words = nouns.get_random_word();
...
let experiment_configuration_function = (writer: Experiment_Output_Writer) => { return {
    experiment_name: "Indentation_Vertical_Jumps_Density_Without_Distance",
    seed: SEED,
    ...
    layout: [
        ...
        { variable: "Body_Lines",           treatments: ["4"] },
    ...
    task_configuration: (t: Task) => {
        ...
        let body_lines = parseInt(t.treatment_value("Body_Lines"));
        ...
        let target_condition_string = if_statement.target_condition_string(level);
        let correct_answer = if_statement.return_string(level);
        let html_string = convert_string_to_html_string(if_statement.print_string(length, body_lines));
        ...
        t.do_print_task = () => {
            ...
            const randomTop  = 3 + Math.floor(Math.random() * 17);
