...
export class Term {
    print_string(indentation_size, empty_lines) {
        let arr = [];
        this.print_into(arr, 0, indentation_size, empty_lines);
        return arr.join("");
    }
    print_string_with_extra_lines(indentation_size, empty_lines) {
        let arr = [];
...
export class Return extends Term {
    ...
    print_into(array, indentation_depth, indentation_length, empty_lines) {
        array.push(" " + this.return_value + "\n");
    }
    print_into_with_extra_lines(array, indentation_depth, indentation_size, empty_lines) {
        array.push(" " + this.return_value + "\n");
    }
}
export class Nested_Ifs extends Term {
    ...
    print_into_with_extra_lines(array, indentation_depth, indentation_length, empty_lines) {
        array.push(" ".repeat(indentation_length * indentation_depth) + "if(" + this.condition_string + ") {\n");
        for (let i = 0; i < empty_lines; i++)
            array.push("\n");
        if (this.then_branch instanceof Return) {
