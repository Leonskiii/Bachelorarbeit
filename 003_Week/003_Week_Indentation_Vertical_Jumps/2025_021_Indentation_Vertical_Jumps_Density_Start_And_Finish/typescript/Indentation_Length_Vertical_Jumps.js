...
let experiment_configuration_function = (writer) => {
    return {
        ...
        layout: [
            { variable: "Length", treatments: ["0"] },
            { variable: "Segment", treatments: ["0", "1", "2", "3", "4"] },
            { variable: "Row", treatments: ["1", "2", "3", "4", "5", "6", "7", "8", "9"] },
            { variable: "Density", treatments: ["0", "1"] },
        ],
        repetitions: 4,
        ...
        task_configuration: (t) => {
            ...
            const segment = parseInt(t.treatment_value("Segment"));
            const row = parseInt(t.treatment_value("Row")) - 1;
            const density = parseInt(t.treatment_value("Density")); // ← aus Treatment lesen
            const { grid, marked_number } = generate_grid(// ← kein density-Return mehr
            num_rows, segment, row, length, density, Math.floor(Math.random() * 100000));
