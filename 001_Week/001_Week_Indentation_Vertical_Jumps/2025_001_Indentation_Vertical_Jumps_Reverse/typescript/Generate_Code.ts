...
export class Nested_Ifs extends  Term {
    ...
    target_return_value(target_number: number): number {
        if(target_number == 1) {
            return (this.else_branch as Return).return_value;
        } else {
            return (this.then_branch as Nested_Ifs).target_return_value(target_number-1);
        }
    }
