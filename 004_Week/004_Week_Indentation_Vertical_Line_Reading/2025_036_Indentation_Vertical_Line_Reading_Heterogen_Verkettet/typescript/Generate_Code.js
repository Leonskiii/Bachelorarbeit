...
export function set_if_conditions_random_length(if_statement) {
    let nouns = new Nouns();
    let starting_letters = [];
    function get_unique_word() {
        let word = nouns.get_random_word();
        let attempts = 0;
        while ((word.length < 2 || word.length > 10 || starting_letters.includes(word[0]))
            && attempts < 2000) {
    ...
    while (if_statement instanceof Nested_Ifs) {
        const use_two_words = Math.random() < 0.4;
        if (use_two_words) {
