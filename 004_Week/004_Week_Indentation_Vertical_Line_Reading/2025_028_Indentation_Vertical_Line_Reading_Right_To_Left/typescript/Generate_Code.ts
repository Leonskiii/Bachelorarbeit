...
export class Nested_Ifs extends Term {
    ...
    print_into_right_to_left(array: string[], depth: number, max_depth: number, length: number) {
        // Äußerste Ebene steht am weitesten rechts, jede tiefere Ebene rückt nach links
        const right_anchor = max_depth * length;
        const col           = right_anchor - depth * length;
        const if_str        = "if(" + this.condition_string + ") {";
        const close_col     = col + if_str.length - 1;
        ...
        if (this.then_branch instanceof Return) {
            ...
            (this.then_branch as Nested_Ifs).print_into_right_to_left(array, depth + 1, max_depth, length);
        }

        array.push(" ".repeat(close_col) + "}");
...
export function set_if_conditions_on_nouns_each_starting_with_different_letter(if_statement: Term) {
    let starting_letters = [];
    let nouns = new Nouns();
    while (if_statement instanceof Nested_Ifs) {
        let word = nouns.get_random_word();
        while (starting_letters.includes(word[0])) word = nouns.get_random_word();
