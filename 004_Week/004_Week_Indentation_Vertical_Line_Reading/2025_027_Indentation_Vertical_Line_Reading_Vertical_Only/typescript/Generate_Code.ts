...
export class Nested_Ifs extends Term {
    ...
    print_into_vertical_only(array: string[], depth: number, length: number) {
        for (let i = 0; i < depth * length ; i++) array.push("\n");
        array.push("if(" + this.condition_string + ") {\n");

        if (this.then_branch instanceof Return) {
            array.push("\n");
        } else {
            (this.then_branch as Nested_Ifs).print_into_vertical_only(array, depth + 1, length);
        }
