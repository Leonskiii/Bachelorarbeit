...
function random_noun_by_length(min: number, max: number): string {
    if (max <= 2) {
...
export function generate_If_Statement(
    ...
    function make_chain(is_target: boolean): { condition_string: string, marked_term: string } {
        let terms: string[] = [];
        for (let i = 0; i < chain_length; i++) {
            if (!contrast) {
                terms.push(random_noun_by_length(5, 5));       // Baseline: alle gleich lang
            } else if (is_target) {
                terms.push(random_noun_by_length(2, 2));    // Ziel: exakt 4 Buchstaben
            } else {
                terms.push(random_noun_by_length(5, 5));    // Andere: exakt 7 Buchstaben
            }
        }
        return {
