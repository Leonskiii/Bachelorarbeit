...
function random_condition(rng: () => number): string {
    const len = 2 + Math.floor(rng() * 4);
    let s = "";
    for (let i = 0; i < len; i++) s += random_char(rng);
    return s;
}
...
export interface GridLine {
    ...
    number:      number;
    is_marked:   boolean;
    left_noise:  string;   // ← neu: Kauderwelsch links der Zahl
    right_noise: string;   // ← neu: Kauderwelsch rechts der Zahl
}
...
export function generate_grid(
    ...
    marked_row: number,
    length:     number,
    density:    number,    // ← neu: Anzahl Kauderwelsch-Zeichen um die Zahl
    seed:       number
): { lines: GridLine[], marked_number: number, distance_from_center: number } {
    ...
    // Zahlpositionen: zufällig rechts, auch ohne direkte Wiederholung
    // Rechter Bereich: von (30 + length*3) bis (30 + length*3 + 20)
    ...
    for (let r = 0; r < num_rows; r++) {
        ...
        const num_col   = num_pool[r];

        // Kauderwelsch links und rechts der Zahl generieren
        let left_noise  = "";
        let right_noise = "";
        for (let i = 0; i < density; i++) left_noise  += random_char(rng);
        for (let i = 0; i < density; i++) right_noise += random_char(rng);

        lines.push({
            indent, condition, num_col,
...
export function grid_to_html(lines: GridLine[]): string {
    const total = Math.max(...lines.map(l => l.num_col + l.right_noise.length + 2));
    ...
    return lines.map(line => {
        ...
        for (let i = 0; i < if_str.length; i++) {
            if (line.indent + i < total) chars[line.indent + i] = if_str[i];
        }

        // Noise + Zahl platzieren
        const noise_start = line.num_col - line.left_noise.length;
        for (let i = 0; i < line.left_noise.length; i++)  chars[noise_start + i]       = line.left_noise[i];
        chars[line.num_col] = line.number.toString();
        for (let i = 0; i < line.right_noise.length; i++) chars[line.num_col + 1 + i]  = line.right_noise[i];
        ...
        if (line.is_marked) {
            ...
            const spaces_after = noise_start - (line.indent + if_str.length);
            row_html += " ".repeat(Math.max(0, spaces_after));
            row_html += line.left_noise;
            row_html += line.number.toString();
            row_html += line.right_noise;
        } else {
            row_html += chars.join("")
                .replace(/&/g, "&amp;")
                .replace(/</g, "&lt;")
